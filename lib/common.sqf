	...
	if( count _expos  > 0) then
	{
		if( _unit != leader _unit 
			...
			&& { 
				...
				|| (_cmd in ['', 'MOVE'] && (_expos select 1) in ["FORMATION PLANNED"]) 
			} 
...
bcombat_fnc_eh_hit = {
	...
	if( _damage > .01 && { _isenemy || isNull _enemy }  ) then 
	{
...
bcombat_fnc_unit_get_destination = {
	...
	if(_distance > 99999) then { _distance = 99999; };
...
bcombat_fnc_unit_can_inspect_pos = {
	...
	if ( 
		...
		//&& !(captive _unit)
		//&& fleeing _unit)
		_pos distance (_wpdata select 0) < (_wpdata select 1) 
		&& (_dstdata select 1) < _pos distance _unit

	) then {
		_ret = true;
	};
	
		//player globalchat format["%1 <--- %2 < %3  - %4 < %5", _ret, _pos distance (_wpdata select 0), (_wpdata select 1) , (_dstdata select 1), _pos distance _unit ];
...
bcombat_fnc_group_get_waypoint = {
	...
	{
		if(_radius == 0 && _wtype in ["MOVE", "HOLD"]) then { 
			_radius = 250; 
		};
		
		if(_radius == 0 && _wtype in ["SAD", "DESTROY", "GUARD", "SENTRY"]) then { 
			_radius = 500; 
		};
	};
	
	if( combatMode _this in  ["YELLOW"] ) then { _radius = _radius / 2; };
		...
		if( [_x] call bcombat_fnc_is_active
			...
			if( !(isPlayer(leader _x)) && { [_x, _unit] call bcombat_fnc_knprec >= 2 } ) then
			{
				...
				if ( _x distance _unit < bcombat_investigate_max_distance 
					...
					&& { [_x, _unit] call bcombat_fnc_knprec > 1}
					&& { time - ( _x getVariable ["bcombat_investigate_time", -30])  > 30  }
...
bcombat_fnc_reveal = {
	...
	private [ "_unit", "_enemy", "_visible", "_p", "_prc",  "_dist", "_ang",  "_rv" ];
	...
	if ( bcombat_allow_reveal ) then
	{
		...
		{
			_rv = ( 4 /  ( ( _dist / 100 ) max 1)  / ( ( _ang / 45 ) max 1 ) ) / ( (count units (group _unit) ) ^ 0.5 ); 
...
bcombat_fnc_investigate = {
	...
	private ["_leader", "_group", "_unit", "_pos", "_rand"];
	
	_leader = _this select 0;
	_pos = _this select 1;
	_group = group _leader ;
	_rand = 1 / (count units _group);
	_cnt = 0;
	...
	
	{
		if( 
			...
			_x domove ( [_pos, ((_leader distance _pos) / 10) max 5 ] call bcombat_fnc_random_pos );
	...
	if( canfire _unit
		// && { _unit != leader _unit }
		//&& { combatMode _unit == "RED" || random 1 <= (_unit skill "general" ) ^ .5 }
		//&& { combatmode _unit in ["RED", "YELLOW"] } 
		&& { !(fleeing _unit) && !(captive _unit) }
		&& { !([_unit] call bcombat_fnc_has_task) }
		// && { _unit call bcombat_fnc_unit_can_inspect_pos }
	...
	) then
	{
		 ...
		 if( _n < (count (units _group)-1) ) then
		 {
			 (units _group) select (_n + 1) dofollow _unit;
		 };
	};*/
};