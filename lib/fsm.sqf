...
bcombat_fnc_fsm_trigger = 
{
	...
	// if( isNil { _penalty } ) then { _penalty = 0; };

	if ( !(isPlayer _unit ) && { [_unit] call bcombat_fnc_is_active } ) then //&& lifestate _unit == "HEALTHY"
	{
		if (_penalty > 5) then {
...
bcombat_fnc_process_danger_queue = 
{
...

	{
		...
		if(  !(_cause in [1,6]) 
			...
			&& { _cause != 0 || count _targets == 0 }
			&& { ( !(_cause in [3,8]) || !isNull(_enemy) ) } 
			&& { ( _unit distance _dangerPos < bcombat_degradation_distance ) }
	...
	if( _penalty > 0 ) then
	{
		...
		{
			//player globalchat format["%1 ### DOUBLE bcombat_fnc_fsm_trigger() call on %2", time, _unit];
		};
	};
